...
exports.addFavoriteMovie = async (req, res) => {
  try {
    ...
    if (err.code === 11000) {
      return res
        .status(400)
        .json({ message: "Цей фільм уже додано до улюблених" });
    }

    res
      .status(500)
      .json({ message: "Помилка при додаванні фільму", error: err.message });
  }
};
...
exports.removeFavoriteMovie = async (req, res) => {
  try {
    ...
    const result = await FavoriteMovie.deleteOne({
      userId: req.userId,
      movieId,
    });
    ...
    res
      .status(500)
      .json({ message: "Помилка при видаленні фільму", error: err.message });
  }
};
...
exports.getFavoriteMovies = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Помилка при отриманні улюблених фільмів",
        error: err.message,
      });
  }
};
...
exports.checkFavoriteMovie = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Помилка при перевірці статусу фільму",
        error: err.message,
      });
  }
};
