<template>
  <div
    class="nav__bar-inner mb-[65px] py-[10px] flex items-center justify-between fixed w-full transition-[top] duration-300 ease-in-out z-[1000] max-w-[1480px] right-0 left-0 mx-auto p-[10px] px-[15px] bg-black"
  >
    <img src="@/assets/images/logo.png" alt="Logo" />
    <!-- {{ userStore }} -->
    <div class="flex items-center gap-[50px] md:justify-between w-full p-[40px] md:p-0 bg-black z-[-1] absolute  md:static top-0 left-0 right-0 flex-col transition-transform md:flex-row md:translate-y-0"
      :class="isBurgerMenuOpen ? 'translate-y-[63px]' : '-translate-y-full'"
    >
      <div/>
      <nav class="nav__bar-navigation">
        <ul class="nav__bar-list flex flex-col items-center md:flex-row gap-[40px]">
          <li class="nav__bar-item border-b-2 border-transparent hover:border-white p-[5px] ">
            <router-link to="/" @click="isBurgerMenuOpen = false">Home</router-link>
          </li>
          <li class="nav__bar-item border-b-2 border-transparent hover:border-white p-[5px] ">
            <router-link to="/discover-films" @click="isBurgerMenuOpen = false">Discover Films</router-link>
          </li>
          <li class="nav__bar-item border-b-2 border-transparent hover:border-white p-[5px] ">
            <router-link to="/my-list" @click="isBurgerMenuOpen = false">My List</router-link>
          </li>
        </ul>
      </nav>
      <div class="nav__bar-control flex flex-col items-center  md:flex-row gap-[32px]">
        <search-bar />
        <router-link
        ...
        >
          ...
          <div v-if="userStore.photo" class="nav__bar-authorized relative">
            ...
            />
            <div
              class="profile__icon-inner w-[16px] h-[16px] bg-white/30 rounded-full absolute left-[-5px] bottom-0 shadow-2xl"
            >
              <svg
          ...
          <div v-else class="bg-white/30 rounded-full">
            <svg
              class="p-[4px] rounded-full shadow-2xl"
              xmlns="http://www.w3.org/2000/svg"
              width="38px"
              height="38px"
              viewBox="0 0 24 24"
    ...
    <button  @click="toogleBurgerMenu" class=" md:hidden  flex gap-[10px] flex-col">
      <span
        class="block w-[30px] h-[3px] bg-white rounded transition-all origin-center duration-300"
        :class="isBurgerMenuOpen ? 'translate-y-[13px] rotate-45' : ''"
      ></span>

      <span
        class="block w-[30px] h-[3px] bg-white rounded transition-all origin-center duration-300"
        :class="isBurgerMenuOpen ? 'opacity-0' : 'opacity-100'"
      ></span>
